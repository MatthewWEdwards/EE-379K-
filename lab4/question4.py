...
y = train.SalePrice
#%% Cross validate ridge and lasso models
...
for fold in range(1, 6):
    ...
    model_ridge = Ridge(alpha=10).fit(folds_train, y_train)
    model_lasso = LassoCV(alphas=[.0007]).fit(folds_train, y_train)    
...
X_train = X_train.drop(["ID", "FoldID", "SalePrice"], axis=1)
normal_model_ridge = Ridge(alpha=10).fit(X_train, y)
normal_model_lasso = LassoCV(alphas=[.0007]).fit(X_train, y)
...
#%% Stack the models
stack_train = stack_train.drop(["ID", "FoldID", "SalePrice"], axis=1)
final_model_ridge = Ridge(alpha=1).fit(stack_train, y)

print(rmse_cv(final_model_ridge).mean())
